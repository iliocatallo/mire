 ...
 * @param {Function} [opt.fallback = throwingHandler] - the fallback handler to be used when no other handler matches the arguments.
 * @return {Function} a generic function.
 **/
function create(opt = {}) {
    const {name, length, fallback} = opt;
    if (fallback && !isFunction(fallback)) {
        throw fallbackNotAFunctionError(fallback);
    }
 ...
 * @return {Function} a generic function.
 **/
function createValidated({name, length, fallback}) {

    const dispatcher = new Dispatcher();

    const generic = function (...args) {
        const handler = dispatcher.getHandler(args.slice(0, length)) || fallback || throwingHandler(generic);
        return handler(...args);
    };
...
/**
 * Constructs a type error to signal that what provided as fallback handler is not a function.
 * @private
 * @param {*} fallback - what has been erroneusly passed as the fallback handler.
 * @return {TypeError} a type error with an informing error message.
 **/
function fallbackNotAFunctionError(fallback) {
    const msg = `Expecting the fallback handler to be a function, got ${inspect(fallback)} instead`;
 ...
 * @return {boolean} whether the argument is in fact a function.
 **/
